...
Handlebars.registerHelper('ifElse', function (input, ifTrue, ifFalse) {
...
Handlebars.registerHelper('underscoreSpaces', function (str) {
    if (!isDefined(str)) {
        return str;
    }
    return str.replace(' ', '_');
});
Handlebars.registerHelper('remove', function (str, filter) {
    if (!isDefined(str)) {
        return str;
    }
    return str.replace(filter, '');
});
Handlebars.registerHelper('nidoranGender', function (str, maleTag, femaleTag) {
...
Handlebars.registerHelper('addFemaleTag', function (pokemon, femaleTag) {
    return (pokemon.gender === Gender.female || pokemon.gender === 'female') && pokemon.hasFemaleSprite ? femaleTag : '';
});
Handlebars.registerHelper('isZero', function (value) {
    return value === 0;
});
const illegalCharacters = /\b(?<!http|https|localhost)\b:|%(?![0-9a-fA-F])|\*/g;
export function resolveIllegalCharacters(input) {
    if (!isDefined(input)) {
        return '';
    }
    if (input.indexOf('?') !== -1) {
...
export class ParamsManager {
    ...
    getBool(key, _default = false) {
        if (!this.hasKey(key)) {
            return _default;
        }
        return this.params.get(key).toLowerCase() === 'true';
    }
    getNumber(key, _default = 1) {
        if (!this.hasKey(key)) {
            return _default;
        }
        let value = this.params.get(key);
...
export function string2ColHex(input) {
