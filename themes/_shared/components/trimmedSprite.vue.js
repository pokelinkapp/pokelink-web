    ...
    methods: {
        ...
        handleFallback() {
            if (this.usePartySprites) {
                V3.usePartyFallback(this.$refs.spriteImg, this.pokemon);
            }
            else {
                V3.useFallback(this.$refs.spriteImg, this.pokemon);
            }
        },
        trim() {
            ...
            img.onerror = () => {
                V3.useFallback(img, this.pokemon);
            };
            img.onload = () => {
        ...
        trimCanvas(c) {
            ...
            const trimHeight = bound.bottom - bound.top, trimWidth = bound.right - bound.left, trimmed = ctx.getImageData(bound.left, bound.top, trimWidth, trimHeight);
    ...
    watch: {
        pokemon: {
            ...
            handler(newVal, oldVal) {
                const spriteHasChanged = this.oldImage !== V3.getSprite(newVal);
                if (spriteHasChanged) {
...
//# sourceMappingURL=trimmedSprite.vue.js.map