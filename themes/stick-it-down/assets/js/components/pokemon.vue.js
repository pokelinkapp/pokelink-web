...
import { V3, V3DataTypes } from 'pokelink';
...
export default defineComponent({
    ...
    computed: {
        ...
        statusClasses() {
            ...
            let statuses = [];
            if (this.pokemon.status === V3DataTypes.StatusEffect.poisoned || this.pokemon.status === V3DataTypes.StatusEffect.badlyPoisoned)
                statuses = [...statuses, 'isPoisoned'];
            if (this.pokemon.status === V3DataTypes.StatusEffect.paralyzed)
                statuses = [...statuses, 'isParalyzed'];
            if (this.pokemon.status === V3DataTypes.StatusEffect.burned)
                statuses = [...statuses, 'isBurned'];
            if (this.pokemon.status === V3DataTypes.StatusEffect.frozen)
                statuses = [...statuses, 'isFrozen'];
            return statuses;
        },
        cellColour() {
            if (!this.pokemonExists || this.pokemon.isEgg)
                return '#120c2f';
