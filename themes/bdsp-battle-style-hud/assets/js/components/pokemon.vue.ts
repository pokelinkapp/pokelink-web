import {defineComponent, PropType} from 'vue'
import {clientSettings, V3, V3DataTypes} from 'pokelink'
import female from './female.vue.js'
import male from './male.vue.js'
import pokeball from './pokeball.vue.js'
import heartGauge from '../../../../_shared/components/heartGauge.vue.js'
import type {Pokemon} from 'pokelink'

export default defineComponent({
    template: `
      <div :class="{ 'pokemon': true, 'opaque': !fixedSprite}" :style="{'opacity': opacity }"
           v-if="!isUndefined(pokemon)">

        <div class="details" v-if="typeof pokemon == 'object'">
          <h2 class="name">
            {{ pokemon.misc?.nickname ?? pokemon.translations.locale.species }}
            <span class="sex" :class="sex" v-if="!hideGender && sex !== '' && !pokemon.isEgg">
            <female v-if="sex === 'female'"></female>
            <male v-if="sex === 'male'"></male>
          </span>
            <span class="lvl" v-if="!hideLevel && !pokemon.isEgg">Lv. {{ pokemon.exp?.level }}</span>
          </h2>
          <div class="hp" v-if="!pokemon.isEgg">
            <h5>HP: </h5>
            <div class="bar">
                <div
                    class="health" :style="{ width: healthPercent }"
                    :class="{ low: parseFloat(healthPercent) <= 50, critical: parseFloat(healthPercent) <= 15 }"
                    >
                </div>
            </div>

          </div>
          <div class="hp__text-and-exp" v-if="!pokemon.isEgg">
            <span class="text">{{ pokemon.hp.current }}/{{ pokemon.hp.max }}</span>

            <heart-gauge v-if="pokemonExists && !hideLevel" :pokemon="pokemon"></heart-gauge>
            <div class="exp" v-if="pokemonExists && !pokemon.isEgg && !hideLevel && !pokemon.isShadow">
              <div
                  :style="{width:experienceRemaining}"
                  :class="{ exp__inner: true}"
              ></div>
            </div>
          </div>
        </div>
        <div v-else></div>
      </div>
    `,
    props: {
        pokemon: {
            type: Object as PropType<Pokemon>,
            required: true
        }
    },
    data() {
        return {
            fixedSprite: true
        }
    },
    components: {
        'female': female,
        'male': male,
        'pokeball': pokeball,
        'heart-gauge': heartGauge
    },
    mounted() {

    },
    computed: {
        pokemonExists() {
            if (!V3.isValidPokemon(this.pokemon)) {
                return false
            }
            return true
        },
        partner() {
            return false
        },
        healthPercent() {
            return (100 / this.pokemon.hp!.max) * this.pokemon.hp!.current + '%'
        },
        nickname() {
            if (!V3.isValidPokemon(this.pokemon)) {
                return null
            }
            return this.pokemon.misc?.nickname || this.pokemon.translations!.english!.species
        },
        sex() {
            switch (this.pokemon.gender) {
                case V3DataTypes.Gender.male:
                    return 'male'
                case V3DataTypes.Gender.female:
                    return 'female'
            }

            return ''
        },
        ident() {
            if (!V3.isValidPokemon(this.pokemon)) {
                return null
            }
            return this.pokemon.species
        },
        opacity() {
            if (!V3.isValidPokemon(this.pokemon)) {
                return '1'
            }
            if (this.fixedSprite) {
                return ''
            }
            return ''
        },
        hasItem() {
            if (!V3.isValidPokemon(this.pokemon)) {
                return false
            }
            if (typeof this.pokemon.misc?.heldItem === 'undefined') {
                return false
            }
            return this.pokemon.misc?.heldItem !== 0
        },
        selectedPokemon: {
            get: function () {
                return this.nickname
            },
            set: function () {
                this.$emit('change', this.nickname)
            }
        },
        hideGender() {
            return clientSettings.params.getBool('hideGender', false)
        },
        hideLevel() {
            return clientSettings.params.getBool('hideLevel', false)
        },
        experienceRemaining() {
            return `${this.pokemon.exp?.nextLevelPercent}%`
        }
    },
    methods: {
        isUndefined(pokemon: Pokemon) {
            return !V3.isValidPokemon(pokemon)
        },
        getSprite(pokemon: Pokemon) {
            return V3.getSprite(pokemon)
        },
        getPokeballTopColor: function () {
            // if (!V3.isValidPokemon(this.pokemon)) { return '#C3C4C6'; }
            //
            // if (settings.pokeImg.pokemonColor === true) {
            //     return this.pokemon.misc?.color;
            // }
            //
            // if (settings.pokeImg.routeColor === true) {
            //     return string2Hex(this.pokemon.locationMet.toString());
            // }
            //
            // if (settings.pokeImg.typeColor === true) {
            //     return getTypeColor(this.pokemon.types[0].label);
            // }
            //
            // if (settings.pokeImg.staticColor !== false) {
            //     return normalizeColor(settings.pokeImg.staticColor, 100);
            // }

            return 'white'
        },
        getPokeballBottomColor: function () {
            // if (settings.champion.misc?.colorPokeball !== true) { return '#C3C4C6'; }
            // if (!V3.isValidPokemon(this.pokemon)) { return '#C3C4C6'; }
            //
            // if (settings.pokeImg.pokemonColor === true) {
            //     if (settings.champion.misc?.colorBothSidesPokeball !== true) { return '#C3C4C6'; }
            //     return this.pokemon.misc?.color;
            // }
            //
            // if (settings.pokeImg.routeColor === true) {
            //     if (settings.champion.misc?.colorBothSidesPokeball !== true) { return 'C3C4C6'; }
            //     return string2Hex(this.pokemon.locationMet.toString());
            // }
            //
            // if (settings.pokeImg.typeColor === true) {
            //     if (this.pokemon.types.length === 2) {
            //         return getTypeColor(this.pokemon.types[1].label);
            //     }
            //     return getTypeColor(this.pokemon.types[0].label);
            // }
            //
            // if (settings.pokeImg.staticColor !== false) {
            //     if (settings.champion.misc?.colorBothSidesPokeball !== true) { return 'C3C4C6'; }
            //     return settings.pokeImg.staticColor;
            // }

            return 'white'
        }

    }
})