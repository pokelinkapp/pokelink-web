...
import { V3, V3DataTypes, hex2rgba, typeColors, string2ColHex, clientSettings, htmlColors, isDefined } from 'pokelink';
...
export default defineComponent({
    template: `
      <div :class="{ 'pokemon': true }" :style="{'opacity': opacity }">
        ...
        <label v-if="isValid">
          ...
          <span class="lvl" v-if="!pokemon.isEgg">Lv. {{ pokemon.exp?.level }}</span>
    ...
    methods: {
        ...
        useFallback() {
            V3.useFallback(this.$refs.pokemonImg, this.pokemon);
        },
        getPokeballTopColor: function () {
            ...
            if (this.settings.pokemonColor) {
                return isDefined(this.pokemon.misc?.color) ? htmlColors[this.pokemon.misc.color.toLowerCase()] : 'white';
            }
            if (this.settings.routeColor) {
                if (isDefined(this.pokemon.translations.locale.locationMet)) {
                    return string2ColHex(this.pokemon.translations.locale.locationMet);
                }
                return 'white';
            }
        ...
        getBorderColor: function () {
            ...
            if (this.settings.pokemonColor) {
                return hex2rgba(isDefined(this.pokemon.misc?.color) ? htmlColors[this.pokemon.misc.color.toLowerCase()] : 'white', 50);
            }
            if (this.settings.typeColor) {
                return typeColors[this.pokemon.translations.english.types[0]];
            }
