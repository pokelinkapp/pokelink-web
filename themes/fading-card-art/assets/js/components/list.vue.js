...
import { V3, clientSettings, collect, isDefined } from 'pokelink';
    ...
    template: `
      <div style="display: none" :class="{ 'browser-connected' : true }" class="pokes">
        <transition-group :name="switchSpeed" tag="div"
                          ...
                          v-if="loaded">
          <pokemon v-for="( poke, idx ) in partySlots" :slotId="idx + 1" :key="poke?.uid ?? poke?.uid ?? poke?.pid" :pokemon="poke" :art="art[idx]">
          </pokemon>
        </transition-group>
      </div>
    `,
                ...
                for (let i = 0; i < filteredParty.length; i++) {
                    ...
                    if (filteredParty[i].isEgg) {
                        ...
                        try {
                            ...
                            console.log(`unknown image for ${filteredParty[i].translations.english.species}`, e);
