...
import {V3, clientSettings, V3DataTypes} from 'pokelink'
...
export default defineComponent({
        ...
        <div class="pokemon" :style="innerMonStyle">
          <label v-if="isValid">
            <div class="hp" v-if="!pokemon.isEgg && hideHPBar">
              <div :style="{height:healthPercent}"
    ...
    computed: {
        flat() {
            return clientSettings.params.getBool('flat', false)
        },
        hideHPBar() {
            return clientSettings.params.getBool('hideHPBar', false)
        },
        isValid() {
            return V3.isValidPokemon(this.pokemon)
        },
        heldItemImage() {
            if (!this.isValid || this.pokemon?.misc?.heldItem === 0) {
                return ""
            }
