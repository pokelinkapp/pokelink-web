Vue.component( "List", {
  ...
  <div style="display: none" :class="{ 'browser-connected' : true }" class="pokes">
    <transition-group
      :name="switchSpeed"
      tag="div"
      :class="['pokemon__list', {'flipped': flipped === true, 'responsive': responsive}]"
      v-if="loaded"
    >
      <Pokemon v-for="( poke, idx ) in party" :key="idx" :pokemon="poke">
      </Pokemon>
    </transition-group>
...
`,
  ...
  created: function () {
    ...
    this.responsive = window.settings.theme.responsive
  },
  mounted: function () {
    var vm = this;
